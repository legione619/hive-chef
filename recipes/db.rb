...
bash 'setup-hive' do
  ...
  code <<-EOH
        ...
        #{node['ndb']['scripts_dir']}/mysql-client.sh -e \"CREATE USER IF NOT EXISTS '#{node['hive2']['mysql_user']}'@'%' IDENTIFIED BY '#{node['hive2']['mysql_password']}'\"
        ...
        #{node['ndb']['scripts_dir']}/mysql-client.sh -e \"GRANT NDB_STORED_USER ON *.* TO '#{node['hive2']['mysql_user']}'@'%'\"
        ...
        #{node['ndb']['scripts_dir']}/mysql-client.sh -e \"GRANT ALL PRIVILEGES ON metastore.* TO '#{node['hive2']['mysql_user']}'@'%'\"
        ...
        #{node['ndb']['scripts_dir']}/mysql-client.sh -e \"GRANT SELECT ON hops.hdfs_inodes TO '#{node['hive2']['mysql_user']}'@'%'\"
        ...
        #{node['ndb']['scripts_dir']}/mysql-client.sh -e \"GRANT REFERENCES ON hops.hdfs_inodes TO '#{node['hive2']['mysql_user']}'@'%'\"
